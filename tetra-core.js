...
(function () {
  ...
  /* ------------------------------------------------------------------ *
   * Анти-FOUC: класс .tetra-anim на <html> ставится синхронным инлайн-
   * скриптом в <head> Webflow и прячет анимируемые элементы до того, как
   * отработают gsap.set() наших модулей. Снимаем его через кадр после
   * DOMContentLoaded — к этому моменту все модули (tetra-page + компоненты)
   * уже проставили стартовые состояния, дальше всем рулит GSAP, флеша нет.
   * В <head> есть и свой failsafe-таймер на случай, если этот файл не
   * загрузился вовсе.
   * ------------------------------------------------------------------ */
  T.ready(function () {
